...
% This code will implement OOK Modulation and compare the theoretical and
% analytical BER 
...
for i=1:length(SNR_dB)
    ...
    for j=1:size
        n(j)=sqrt(1/2)*M*randn(); % using randn function to randomly choose any integer
    end
    ...
    yn=zeros(1,size);
    for j=1:size
        if(y(j)>=0.5) % Based on decision rule , either +V(1) or 0 is choosen
            yn(j)=1;
        else
            yn(j)=0;
        end
    ...
    BER_Theoretical(i)=qfunc(sqrt(1/N));  % Calculation of Theoretical BER using Q function
...
title('19ucc023 - Mohit Akhouri','Plot of Theoretical and Practical BER vs. SNR(dB) for OOK modulation');
grid on;
